...
2. **微旋转精修自检（非循环）** —— `./build/karpenko_rot_selfcheck`。注入 3.57° 的相机-陀螺
   非轴对齐失配 `P_true=expSO3(tilt)`，按物理约定生成观测。离散枚举只能取到最近轴排列、残留 tilt
   会偏置 ts；开 `--refine-rot` 后应解回。**实测：离散 ts 误差 172µs → 精修 2µs，td 精确恢复（PASS）。**
   证明 `refine_rotation` 的连续 so(3) 精修实现正确。

3. **全局快门 (GS) 真实数据** —— 反解 `t_RS(ts)` 应 ≈ 0。

4. **卷帘 (RS) 交叉核对** —— 与已知参考量级对比：
...
**精度提升项（1/2/4 已实现为 opt-in 开关，合成集验证有效）**

1. **鲁棒核（Huber）** —— `--huber <px>`：残差改 Huber 损失，降权近景大视差外点。
   （见下方 seq1 对比：在**平移主导**数据上会因外点比例过高而过拟合，故默认关。）
2. **连续 R_cam_gyro 微旋转精修** —— `--refine-rot`：在最优离散轴排列附近再优化 3 维 so(3)
   微旋转 `R_eff = expSO3(δ)·P_base`，去掉 TUM `T_SC` ~0.6° 非轴对齐的系统偏置。
   **合成验证（`synthetic_rot_selfcheck`）：注入 3.57° 失配，离散枚举 ts 误差 172µs → 精修后 2µs，td 精确恢复。**
   这是**旋转主导**数据上对**绝对** ts 增益最大的一项。
3. **旋转主导片段选择**：EIS 假设旋转为主；自动挑选高角速率/低平移窗口（或用 IMU 加计估平移量级）喂标定，
   可让**绝对** ts 逼近差分值（尚未落地）。
4. **前端可选单应 RANSAC** —— `--homography`：纯旋转+远景下 F 退化，单应 `K·R·K⁻¹` 与假设自洽
   且天然剔大视差点。默认用基础矩阵 F（seq1 含平移，对极几何良定义）。
...
**seq1 上四变体对比（差分 t_RS，真值 0.03018 s）**

| 前端 / 鲁棒 / 精修 | cam1 ts | cam0 ts | 差分 | 误差 | cam1 RMS | 备注 |
|---|---|---|---|---|---|---|
| **F + 平方 + 枚举（默认）** | 0.05387 | 0.02330 | **0.03057** | **+1.28%** | 17.36 | 复现基线，对齐 Ctrl-VIO |
| 单应 + 平方 + 枚举 | — | — | ~0.0288 | −4.6% | 15.3 | RMS 更低但差分偏低 |
| 单应 + 平方 + 精修 | — | — | ~0.0291 | −3.7% | — | 精修在平移数据上无增益 |
| 单应 + Huber(3px) + 枚举 | — | — | ~0.0229 | −24% | — | 仅 16% 内点，过拟合视差 |

> **结论（如实说明）**：
> - **正确性**：修正手性后自研方法正确——非循环合成自检 0 误差、微旋转精修合成集 172µs→2µs、
>   real 差分 +1.28% 对齐 Ctrl-VIO、td 双目一致。
> - **精度**：seq1 为**平移视差受限**——差分 t_RS 在 ±5% 带内随前端/鲁棒选择波动，**默认 F 矩阵的 +1.28%
>   已是本数据上的最优**；额外自由度（单应/Huber/精修）在此会去拟合视差而非卷帘信号，故全部默认关闭。
> - 三个提升开关（`--refine-rot`/`--huber`/`--homography`）的**有效域是旋转主导/远景数据**
>   （合成集已验证 refine-rot 显著有效）；在平移主导的 seq1 上不启用，以保持共模视差的干净可抵消性。
