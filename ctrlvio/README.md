...
## 验证判据
| 输入 | 相机 | 期望 line_delay(每行) | 期望整帧 t_RS |
|---|---|---|---|
| TUM-RSVI `/cam1`（右目） | 卷帘 RS | ≈ **29.47 µs** | ≈ **0.03018 s** |
| TUM-RSVI `/cam0`（左目，改 cam_yaml 的 image_topic 试） | 全局 GS | ≈ 0 | ≈ 0 |

上游报告：从 0 初值约 1 秒内收敛，WHU 合成集标定误差 ~0.02–3 µs，对初值鲁棒。
