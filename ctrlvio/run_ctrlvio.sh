...
# 用法：
#   cd ctrlvio
#   ./run_ctrlvio.sh <bag相对datasets的路径>        # 默认 tum_rsvi/dataset-seq1.bag
#   例：./run_ctrlvio.sh tum_rsvi/dataset-seq1.bag
#
...
BAG_REL="${1:-tum_rsvi/dataset-seq1.bag}"
BAG_HOST="$ROOT/datasets/$BAG_REL"
if [ ! -f "$BAG_HOST" ]; then
...
TAG="$(basename "$BAG_REL" .bag)"
OUT="$ROOT/results/ctrlvio/$TAG"
mkdir -p "$OUT"

# 容器内：起 roscore → rosrun 节点（自己离线读 bag）→ 输出 tee 到 results
$DOCKER run --rm \
  -e config_path=/work/config/ct_odometry_tumrs.yaml \
  -v "$ROOT/ctrlvio/config":/work/config:ro \
  -v "$ROOT/datasets":/work/datasets:ro \
  -v "$OUT":/work/results \
  "$IMAGE" \
  bash -lc '
    ...
    rosrun ctrlvio odometry_node \
      _config_path:=/work/config/ct_odometry_tumrs.yaml \
      _bag_path:=/work/datasets/'"$BAG_REL"' \
